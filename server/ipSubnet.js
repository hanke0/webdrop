...
export function getClientIp(req) {
    ...
    const fromForwarded = typeof forwarded === 'string'
        ...
        : undefined;
    const raw = (typeof xReal === 'string' && xReal.length > 0 ? xReal : undefined) ??
        fromForwarded ??
        req.socket.remoteAddress ??
        '';
    return typeof raw === 'string' ? raw.trim() : '';
}
function stripZoneAndV4Mapped(raw) {
    let ip = raw.trim();
 ...
 */
export function subnetSeedForLanRoom(ipRaw) {
...
export function roomCodeFromSeed(seed) {
