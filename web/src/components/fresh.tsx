...
export type FreshProps = {
  ...
  height: number
  className?: string
  /** When set, called on click; internal spin + debounce apply. */
  onClick?: () => Promise<void>
  /** When true, icon spins (e.g. parent-triggered refresh). */
  busy?: boolean
}

export function Fresh(props: FreshProps) {
  const [clicking, setClicking] = useState(false)
  const spinning = clicking || (props.busy ?? false)
  ...
  return (
    <svg
      ...
      className={`${props.className ?? ''} ${spinning ? 'animate-spin' : ''} ${props.onClick ? 'cursor-pointer' : ''}`}
      onClick={
        props.onClick
          ? async () => {
              if (clicking) {
