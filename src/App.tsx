        ...
        {!room.isLoading && sPeer ? (
          <Card>
            ...
            <div className="h-full items-center justify-center">
              ...
              {sSelectedItem && (
                <Dialog closeable={true} onClose={onCloseSelectItem}>
                  <h3 className="py-3 px-3 text-[20px]">
                    <span className="text-[20px]">Send to</span>
                    <div className="inline border rounded-lg px-1 mx-1">
                      <UserHead
                        fullName={sSelectedItem.value.fullName}
                        className="inline text-center mb-1"
                        width={20}
                        height={20}
                      />
                      <span className="text-[20px]">
                        {sSelectedItem.value.fullName}
                      </span>
                    </div>
                  </h3>
                  <Upload callback={onFileChange}></Upload>
                  {sFile && <div className="py-3 px-3">{sFile.name}</div>}
